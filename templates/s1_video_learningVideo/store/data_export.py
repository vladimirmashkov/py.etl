...
import s1_video_learningVideo.config as pkg
from s1_video_learningVideo.store import data_export_pb2 as pb2
...
def save_to_bin(df_id, df):
    table = pb2.export_s1_video_learningVideo()
...
def read_from_files(file_name):
    with open(file_name, "rb") as f:
        table = pb2.export_s1_video_learningVideo().FromString(f.read())
...
def dst_put(dict_obj):
    ...
    dic1 = dict_obj["row"]
    # print(dic1)
    pg_ext.execute_batch (cursor, """INSERT INTO ss.s1_video_learningVideo (id, created_at, updated_at, uuid, url, "name", description, image, "number", duration_min, is_popular, class_id, subject_id, access_id, sfDateTime, sfHash, sfPackageId, sfPackageFlowId, sfTableoid, sfXmin, sfFormerDateTime, sfChangeState) VALUES (
